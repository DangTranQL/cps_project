...
def movebase_server():
    ...
    goal.target_pose.header.frame_id = 'map'
    goal.target_pose.header.stamp = rospy.Time.now()
    goal.target_pose.pose.position.x = 0.5177749224001301
    goal.target_pose.pose.position.y = -0.7754229225708715
    goal.target_pose.pose.orientation.z = 0.709802492894545
    goal.target_pose.pose.orientation.w = 0.704400753180098
    client.send_goal(goal)
    wait = client.wait_for_result()
...
if __name__ == "__main__":
    ...
    msg.pose = Pose(Point(0.0, 0.0, 0.0), Quaternion(0.000,0.000,0.0016757180975,0.999998595983))
